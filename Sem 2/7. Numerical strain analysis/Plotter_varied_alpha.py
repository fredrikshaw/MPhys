...
def plot_alpha_sweep(peak_data):
    points_by_level = {}

    for level_key, entry in peak_data.items():
        if not entry.get("success", False):
            continue

        params = entry.get("parameters", {})
        annihilation = params.get("annihilation") or level_key.split(" [", 1)[0]
        ...
        if alpha is None or omega is None or not np.isfinite(alpha) or not np.isfinite(h_log):
            continue
        if omega <= 0:
            continue

        points_by_level.setdefault(annihilation, []).append((alpha, omega * (1 / 4.135667696e-15), h_log))

    if not points_by_level:
        raise RuntimeError("No valid data points to plot.")

    plt.figure(figsize=(9, 7))
    ax = plt.gca()
    cmap = plt.get_cmap("viridis")
    alpha_values = [point[0] for points in points_by_level.values() for point in points if np.isfinite(point[0])]
    norm = Normalize(vmin=min(alpha_values), vmax=max(alpha_values))

    for level_label, points in sorted(points_by_level.items()):
        points.sort(key=lambda item: item[0])
        alpha_arr = np.array([point[0] for point in points], dtype=float)
        omega_arr = np.array([point[1] for point in points], dtype=float)
        h_arr = np.array([point[2] for point in points], dtype=float)
        ...
        if len(points) == 1:
            ...
            segments = np.stack(
                [
                    np.column_stack([omega_arr[:-1], h_arr[:-1]]),
                    np.column_stack([omega_arr[1:], h_arr[1:]]),
                ],
                axis=1,
            )
            segment_alpha = 0.5 * (alpha_arr[:-1] + alpha_arr[1:])
            ...
            line_collection.set_array(segment_alpha)
    ...
    plt.title("Peak strain vs frequency for alpha sweep (points ordered by increasing alpha)")
...
if __name__ == "__main__":
    ...
    peak_data = scan_annihilations_and_save(
        output_pickle=f"ann_peak_data_alpha_over_l_{[str(alpha_over_ls).replace('.', 'p') if len(alpha_over_ls) == 0 else 'sweep'][0]}.pkl",
        annihilations=available_annihilations,
        bh_mass_sm=bh_mass_sm,
        bh_spin=bh_spin,
